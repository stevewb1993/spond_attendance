...
selected_categories = st.sidebar.multiselect(
    "Session type", all_categories, default=[c for c in all_categories if c not in ("NA", "Other")]
)
...
if len(date_range) == 2:
    filtered = filtered[
        (filtered["session_date"].dt.date >= date_range[0])
        & (filtered["session_date"].dt.date <= date_range[1])
    ]
...
col4.metric("Date range", f"{filtered['session_date'].min():%b %Y} – {filtered['session_date'].max():%b %Y}")
...
with tab_monthly:
    monthly_session_dow = (
        filtered.groupby(["year_month", "session_name", "session_day_of_week", "category"])["attended"]
        .mean()
        .reset_index()
        .rename(columns={"attended": "avg_attended"})
        .sort_values("year_month")
    )
    monthly_session_dow["label"] = monthly_session_dow["session_name"] + " (" + monthly_session_dow["session_day_of_week"] + ")"
    ...
    for cat in ["Swim", "Bike", "Run", "S&C"]:
        ...
        fig = px.line(
            ...
            color="label",
            markers=True,
            labels={"year_month": "Month", "avg_attended": "Avg attendance", "label": "Session (Day)"},
        )
...
with tab_yoy:
    yoy = (
        filtered.groupby(["year", "month", "session_name", "session_day_of_week", "category"])["attended"]
        .mean()
        .reset_index()
        .rename(columns={"attended": "avg_attended"})
    )
    ...
    for cat in ["Swim", "Bike", "Run", "S&C"]:
        ...
        for session_label in sorted(cat_data["label"].unique()):
            ...
            fig = px.line(
                ...
                markers=True,
                title=session_label,
                labels={"month_name": "Month", "avg_attended": "Avg attendance", "year": "Year"},
...
with tab_detail:
    ...
    detail = (
        filtered.groupby(["session_name", "category", "session_day_of_week"])["attended"]
